...
`
  ...
  function render(ast) {
    ...
    if(ast.body && !(ast.left || ast.right) && !(ast.test || ast.init || ast.update)){
      ...
      if((ast.up.params || ast.params) && (ast.params || '').length > 0){ // treat parameters as first line.
        render.list(ast, ast.up.params || ast.params, '(', ',', ') ' + String.fromCharCode(8594));
      }
  ...
  render.actify = function (ast) {
    if (ast.left && ast.right) {
      ...
      act = { left: ast.specifiers, operator: '->', right: ast.source };
