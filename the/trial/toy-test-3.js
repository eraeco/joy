...
obj.method(c).chain().calls(b);
  ...
  const ASTGroups = {
    ...
    Patterns: {
      types: ['ObjectPattern', 'ArrayPattern', 'RestElement', 'AssignmentPattern', 'SpreadElement'],
      handler: 'pattern'
    },
    Modules: {
      types: [
  ...
  render.list = function(ast, items, opt = {}) {
      if (ast.elements) { 
          opt.s = opt.s || '[';
          opt.b = opt.b || ',';
          opt.e = opt.e || ']';
  ...
  render.body = function(ast) {
    ...
    if (ast.up?.params || ast.params || ast?.param) {
      render.list(ast, ast.up.params || ast.params || ast.param, { s: '(', b: ',', e: ')' });
    }
  ...
  render.flow = function(ast) {
    ...
    if (ast.type === 'IfStatement') {
      render.list(ast, [ast.test], { s: '(', e: ')' });
      render.list(ast, [ast.consequent]);
      if (ast.alternate) {
        render.list(ast, [ast.alternate], { s: 'else' });
      }
    } else if (ast.type === 'SwitchStatement') {
      render.list(ast, [ast.discriminant], { s: '(', e: ')' });
      render.list(ast, ast.cases);
    } else if (ast.type === 'TryStatement') {
      render.list(ast, [ast.block]);
      if (ast.handler) {
        render.list(ast, [ast.handler], { s: 'catch' });
      }
      if (ast.finalizer) {
        render.list(ast, [ast.finalizer], { s: 'finally' });
      }
  ...
  render.declare = function(ast) {
    ...
    if (ast.id && (ast.init || ast.body || ast.params)) {
        ...
        // Add any additional properties based on declaration type
        if (ast.params) {
    ...
    if (ast.source) {
        render.list(ast, items, opt);
        render.list(ast, [ast.source], { s: 'from' });
    } else {
        render.list(ast, items, opt);
    }
  ...
  render.pattern = function(ast) {
    ...
    if (ast.properties || ast.elements) {
        ...
        return;
      } else if (ast.type === 'RestElement' || ast.type === 'SpreadElement') {
        render.value(ast, '...', 'operator');
        render.side(ast.argument, ast, 'right');
        return;
      } else if (ast.type === 'AssignmentPattern') {
        render.side(ast.left, ast, 'left');
        render.value(ast, '=', 'operator');
        render.side(ast.right, ast, 'right');
        return;
      }
  };
  ...
  render.module = function(ast) {
    ...
    if (ast.type === 'ImportDeclaration') {
      ...
      if (ast.specifiers.length) {
        ...
        if (defaultSpecifier) {
          render.side(defaultSpecifier.local, ast, 'specifier');
        }
        ...
        if (namedSpecifiers.length) {
          render.list(ast, namedSpecifiers, {
            s: defaultSpecifier ? ', {' : '{',
            b: ',',
            e: '}'
          });
      ...
      render.value(ast, 'export default', 'keyword');
  ...
  render.act = function(ast) {
    ...
    if (ast.type === 'Pattern' || ast.type === 'ObjectPattern' || ast.type === 'ArrayPattern') {
      ...
      act = { left: ast.object, operator: ast.computed ? '[' : '.', right: ast.property };
    } else if (ast.type === 'AssignmentPattern') {
      act = { left: ast.left, operator: '=', right: ast.right };
    } else if (ast.type === 'ReturnStatement') {
      act = { operator: 'return', right: ast.argument };
    } else if (ast.type === 'Property') {
      act = { left: ast.key, operator: ast.kind === 'init' ? ':' : ast.kind, right: ast.value };
    } else if (ast.body) {
  ...
  render.chain = function(ast) {
    ...
    // First collect all parts of the chain
    while (current) {
    ...
    chainParts.forEach((part, index) => {
        ...
        if (part.type === 'CallExpression') {
            ...
            // Handle arguments
            if (part.arguments.length) {
                render.list(part, part.arguments, {
                    s: '(',
                    b: ',',
                    e: ')'
                });
  ...
  render.side = function (node, ast, path, side = 'right') {
    ...
    if (node.type === 'CallExpression') {
      ...
      if (isChain) {
          ...
          render.list(node, node.arguments, { s: '(', b: ',', e: ')' });
      }
      return;
    } else if (node.type === 'Identifier' || node.type === 'Literal' || node.raw) {
      render.the({
        name: ast.$id + '-' + side,
        sort: [0.1, ast.$id]
      }).the({
      ...
      render.list(ast, node.properties, { s: '{', b: ',', e: '}' });
